...
// Build or copy MCP servers for Codex sessions (one-time, no watch needed)
// If source files exist, build from source. Otherwise, copy pre-built bundles from resources.
async function buildMcpServers(): Promise<void> {
  ...
  const sessionSourceExists = existsSync(join(ROOT_DIR, "packages/session-mcp-server/src/index.ts"));
  const bridgeSourceExists = existsSync(join(ROOT_DIR, "packages/bridge-mcp-server/src/index.ts"));

  if (sessionSourceExists && bridgeSourceExists) {
    // Build both servers from source in parallel
    ...
    // Source not available (OSS repo) — copy pre-built bundles from resources
    const sessionResource = join(RESOURCES_DIR, "session-mcp-server/index.js");
    const bridgeResource = join(RESOURCES_DIR, "bridge-mcp-server/index.js");

    if (existsSync(sessionResource)) {
      cpSync(sessionResource, SESSION_SERVER_OUTPUT);
      console.log("✅ Session MCP server copied from pre-built resources");
    } else {
      console.warn("⚠️  Session MCP server not available (no source or pre-built bundle)");
    }

    if (existsSync(bridgeResource)) {
      cpSync(bridgeResource, BRIDGE_SERVER_OUTPUT);
      console.log("✅ Bridge MCP server copied from pre-built resources");
    } else {
      console.warn("⚠️  Bridge MCP server not available (no source or pre-built bundle)");
    }
  }
}
